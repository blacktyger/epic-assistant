 ...
 *
 * @param {{core: string, canary?: string, tools?: boolean}} opts
 ...
 */
export function buildSystemPrefix({ core, canary = CANARY, tools = false }) {
  return [
    ROLE,
    '',
    core,
 ...
 */
export function buildPrompt({ core, documents, history = [], question, cacheTtl = '5m', canary = CANARY, tools = false }) {
  const system = [
    { text: buildSystemPrefix({ core, canary, tools }) },
...
from general knowledge.${tools ? ' If the question is about the live chain or a GitHub repository, use a tool instead.' : ''}
  ...
  messages.push({
    role: 'user',
    content: [{ text: question + (tools ? TURN_REMINDER_WITH_TOOLS : TURN_REMINDER) }],
  });
 ...
 */
export function buildToolResultTurn(results) {
