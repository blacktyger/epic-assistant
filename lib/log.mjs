...
export class QuestionLog {
  ...
  write(record) {
    ...
    const line = {
      ...
      /*
       * Live-data telemetry: which tools ran, how many rounds it took, and how long each call took.
       *
       * Present as one object rather than flattened fields, and omitted entirely when nothing was
       * called, so the line for an ordinary documentation question is the same size it always was.
       * This is the field that answers the questions the feature will actually raise: how often the
       * model reaches for live data, whether it picks the right tool, and whether the node or GitHub
       * is the slow half.
       *
       * Note for whoever adds the next field here: this writer takes a fixed list of keys rather than
       * spreading the record, so a new key passed by server.mjs is dropped without complaint. That
       * happened to this one, and the only symptom was an absent field in a file nobody reads
       * hourly.
       */
      tools: record.tools?.calls ? record.tools : undefined,
      liveData: record.tools?.calls ? true : undefined,
