 ...
 * site. So the guards target the outputs that can actually harm somebody.
 *
 * There are two of those, and the second arrived with the live-data tools.
 *
 * A link. This is a cryptocurrency project, and an invented wallet download URL is a real user-harm
 * event in a way that a jailbroken poem is not. The allowlist is the highest-value control here.
 *
 * A hostname. The chain tools read from a node we operate, using our credential, and that node also
 * serves an unauthenticated /v2/foreign accepting push_transaction and submit_block. Its name is
 * therefore closer to a credential than to a citation, and the link allowlist cannot catch it, because
 * a bare hostname in prose is not a URL. `redactInternal` is the gate; the <live-data> instruction
 * telling the model not to name it is only advice.
 */
...
/**
 * Removes our own infrastructure hostnames from text.
 *
 * Matched case-insensitively and with the dots escaped, and applied to a URL as well as to a bare
 * mention, so `https://node.example/v1/status` collapses rather than leaving a broken path behind. The
 * replacement is a description rather than a marker like [redacted], because the sentence around it
 * has to survive: "read from a node we run" reads correctly where "read from [redacted]" reads like a
 * bug.
 *
 * `hosts` is a parameter rather than only a config read so a test can state its own list. Testing this
 * against the real configured hostname would either put that hostname in a committed test file or
 * leave the function untested, and neither is acceptable for a control whose whole job is keeping a
 * name out of published text.
 */
export function redactInternal(text, hosts = guardCfg.redactHosts) {
  if (!hosts.length || !text) return { text, findings: [] };

  const findings = [];
  let out = text;
  for (const host of hosts) {
    const escaped = host.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    // The whole URL when it is one, so no orphan scheme or path is left behind.
    const asUrl = new RegExp(`\\b[a-z][a-z0-9+.-]*://${escaped}\\S*`, 'gi');
    const bare = new RegExp(`\\b${escaped}\\b(?::\\d+)?`, 'gi');
    const before = out;
    out = out.replace(asUrl, 'a node we run').replace(bare, 'a node we run');
    if (out !== before) findings.push(`internal-host-redacted:${host}`);
  }
  return { text: out, findings };
}

/**
 * Length of the longest trailing fragment of `buf` that is a proper prefix of any redacted host.
 *
 * This is what makes the redaction safe on a stream. Deltas split wherever the model's tokeniser
 * happens to split, so `node.btlabs.uk` can arrive as `node.bt` then `labs.uk`, and a per-delta regex
 * sees neither. Holding back the ambiguous tail until it either completes the hostname or proves it
 * cannot is the only correct answer; the visible cost is a few characters of extra latency on text
 * that happens to end in `n`, `no`, `nod` and so on.
 */
export function holdForRedaction(buf, hosts = guardCfg.redactHosts) {
  if (!hosts.length) return 0;
  const lower = buf.toLowerCase();
  let hold = 0;
  for (const host of hosts) {
    const max = Math.min(host.length - 1, lower.length);
    for (let len = max; len > hold; len -= 1) {
      if (host.startsWith(lower.slice(lower.length - len))) {
        hold = len;
        break;
      }
    }
...
export class OutputGuard {
  ...
  push(delta) {
    ...
    const { text, findings } = sanitiseReleased(ready);
  ...
  finish() {
    ...
    const { text, findings } = sanitiseReleased(this.#buffer);
    this.#buffer = '';
    this.#findings.push(...findings);
    this.#released += text;
    return { emit: text };
...
/**
 * Everything applied to text on its way out, in the order that matters.
 *
 * Redaction runs before link filtering, not after. A URL pointing at our node would be stripped by the
 * allowlist anyway, since the host is not on it, but that leaves a `[link removed]` marker where a
 * readable phrase belongs and tells the reader something was there. Collapsing it to "a node we run"
 * first produces a sentence, and the allowlist then never sees the URL at all.
 */
function sanitiseReleased(text) {
  const redacted = redactInternal(text);
  const linked = sanitiseLinks(redacted.text);
  return { text: linked.text, findings: [...redacted.findings, ...linked.findings] };
}

/**
 * How much of the buffer is safe to release.
 *
 * Holds back a trailing fragment that could still become a markdown link, a bare URL, or one of the
 * hostnames that must never be published. Anything before the last unmatched '[', the last
 * scheme-shaped token, or an ambiguous hostname prefix is releasable.
 */
export function flushable(buf) {
  ...
  // A hostname arriving one token at a time.
  const forHost = holdForRedaction(buf);
  if (forHost > 0) hold = Math.min(hold, buf.length - forHost);
