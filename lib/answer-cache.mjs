/**
 * Cache of complete answers, keyed on the normalised question.
 *
...
export class AnswerCache {
  ...
  #key(question, modelId) {
    return createHash('sha256')
      .update(`${this.#version}\u0000${modelId}\u0000${normaliseQuestion(question)}`)
      .digest('base64url');
  }

  get(question, modelId) {
    if (!cfg.enabled) return null;
    const key = this.#key(question, modelId);
